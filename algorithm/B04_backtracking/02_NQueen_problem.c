...
int isValid(int len, const int (*arr)[len], int row, int col){
...
void DFS(int i, int len, int (*arr)[len] ) { //第i行
...
int main(){
    ...
    int (*chessStatus)[len]=(int (*)[len])malloc(sizeof(int)*len*len);
    memset(chessStatus, 0, sizeof(int)*len*len); //该区域每个字节都设置为0
